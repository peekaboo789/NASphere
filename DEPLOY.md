...
- 版本：`package.json` 的 `version`（当前 `1.0.6`）。`deploy.sh` 里那个默认标签是单独写死的一行（`DEFAULT_TAG`），发新版时跟着改。
...
| 能拉 `ghcr.io` | 三条路线里只有 §5 不需要联网拉镜像。§3 和 §4 都是 `docker pull ghcr.io/peekaboo789/nasphere:1.0.6`，不构建、不下源码，所以也不用管 `node:22-alpine` 那个基础镜像 |
...
| `镜像：ghcr.io/peekaboo789/nasphere:1.0.6` / `取镜像方式：docker pull（本机不需要源码，也不构建）` / `容器：compose 项目 nasphere → 容器名 nasphere-nasphere-1` / `端口：18086 → 容器内 18086` / `数据：…/data` / `时区：Asia/Shanghai` | 生效的参数，优先级是**命令行 > 环境变量 > `.env` > 默认值**；数据目录已转成绝对路径 |
...
| `加载离线 Docker 镜像：…` → `已把包内镜像 local/nasphere:1.0.6 重标为 ghcr.io/peekaboo789/nasphere:1.0.6` | 仅 `--tar`：包里的名字与目标标签不一致时自动重标，走这条就不会去拉 ghcr |
...
NAS 到 `ghcr.io` 不通（没网、或被墙）就走 §5 的离线包。包里的名字是 `local/nasphere:1.0.6`，和 compose 写的那个不同名，`load` 完补一步重标，之后 compose 见本地已有同名镜像就不会再去拉：

```bash
docker load -i nasphere-1.0.6-linux-amd64.tar.gz
docker tag local/nasphere:1.0.6 ghcr.io/peekaboo789/nasphere:1.0.6
docker compose up -d
```
...
```bash
# 任何有 Docker 的电脑上
./make-image.sh                       # → dist/nasphere-1.0.6.tar.gz 和 .sha256
ssh 你@NAS 'mkdir -p /volume1/docker/NASphere/dist'
scp dist/nasphere-1.0.6.tar.gz* 你@NAS:/volume1/docker/NASphere/dist/

# NAS 上，在安装目录里
cd /volume1/docker/NASphere
sha256sum -c dist/nasphere-1.0.6.tar.gz.sha256
./deploy.sh --tar dist/nasphere-1.0.6.tar.gz
```
...
> 不写 `--tag` 就是重跑当前默认标签（`1.0.6`）。ghcr 上同一个标签被重推过时，这次 pull 会拉回新的那份；想让升级有确定的落点，就给每个版本一个新标签。
...
- **手动**：脚本每次都会把当时的在用镜像额外打个 `<IMAGE>:rollback` 标签（默认就是 `ghcr.io/peekaboo789/nasphere:rollback`），所以
  ...
  cd <安装目录>
  docker compose down
  docker tag ghcr.io/peekaboo789/nasphere:rollback ghcr.io/peekaboo789/nasphere:1.0.6
  docker compose up -d
  ```
...
| 现象 | 先查 | 处置 |
| --- | --- | --- |
| `这台机器上没有 docker` | 脚本要求 PATH 里有 docker | 用 Docker 图形套件里的 Compose/项目功能：先弄出 `ghcr.io/peekaboo789/nasphere:1.0.6` 这个镜像（§5 的离线包导入 + §4 那两行重标），再把 §4 那份 `docker-compose.yml` 贴进项目起起来；卷路径写绝对路径。或按 README「本地开发调试」直接跑 `node server/index.js` |
...
| 硬盘那一行写着「这台机器上没有 /sys/block」 | 容器里看不见宿主机的 sysfs（`/sys` 被挡在外面，或这台机器根本走的是不走 sysfs 的存储栈） | `docker exec <容器> ls /sys/block` 看一眼；被挡就别用 `--pid=host` 之类的隔离开关去改，这页只读文件不读内容 |
| 硬盘那一行写着「/sys/block 里没有可用的盘」 | 看得见 sysfs，但里面的设备全被排除名单滤掉了（`loop` / `ram` / `zram` / `sr` / `md` / `dm-` 这些都不是裸盘），或者是盘名很怪的存储栈没报 `size` | 宿主机上 `ls /sys/block` 和 `cat /sys/block/<盘>/size` 各看一眼，把结果发出来才好判断该不该放宽名单 |
| GPU 那一行写着「读不到 GPU 占用（驱动 … · 频率 …）」 | 这块卡的驱动不往 sysfs 写百分比（核显的 i915 最常见，括号里给的就是驱动名和当前 / 峰值频率） | 这一版没有第二种取值来源，只能显示频率；想省地方就把这一行取消勾选 |
...
- **NAS 资源读数卡只取计数器**：内存来自 `/proc/meminfo` 的两个计数，CPU 是 `/proc/stat` 第一行的累计 ticks 与 `/proc/loadavg`，网络是 `/proc/net/dev` 的收发字节（回环、`docker0`、`veth` 不计），显卡先读 `/sys/class/drm/*/device/gpu_busy_percent`，读不到再退到每个引擎的 `busy_percent`（i915 只写后者），两处都没有就在卡片上写驱动名和当前 / 峰值频率，物理盘是 `/sys/block` 里排除掉虚拟设备之后剩下的设备名 / 型号 / 块数，卷用量是对目录本身调一次 `statfs()`。这些来源都给不出一个文件名，接口返回里也没有挂载路径和网卡名。**代价在卷那一头**：容器默认就能看见数据目录那一卷；`deploy.sh` 每次部署还会探一遍存储池，把探到的每一卷各写一行 `- /volumeN:/host/volumeN:ro` 进它生成的 compose，于是**容器进程技术上能读那些卷里的文件内容**，哪怕页面只显示数字。不接受这条就把 `HOST_VOLUMES=none` 写进 `.env` 重跑，只留数据目录那一卷。
