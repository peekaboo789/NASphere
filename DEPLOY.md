...
| SSH 或图形终端 | SSH 最省事。只有网页图形界面也能装：在 DSM/UGOS 的 Compose 表单里照 §4 粘 `docker-compose.yml`，卷路径写成绝对路径，并先把镜像弄到这台机器上（§3 的 `deploy.sh` 或 §5 的离线包） |
| 首次构建能上网 | 要拉一次 `node:22-alpine`（约 50MB）。NAS 拉不动就用路线 C 的离线镜像包（电脑上没 Docker 也能出，见 §5 C-2） |
| 端口没被占用 | 默认对外 `18086`（`deploy.sh --port` 或 `.env` 里 `HOST_PORT` 可改，手工 compose 就改 `docker-compose.yml` 的 `ports`），容器内固定监听 `18086`。`8080` 常被设备自带服务占用，所以默认值避开它；再冲突就换端口（见 §7） |
...
> 走 §3 的一键安装可以跳过这一节：没有 `.env` 也能装，首装还会自动随机生成初始密码（见 §2 末尾和 §3 的输出对照）。这一节是给 `deploy.sh` 用的——手工 compose（§4）不读 `.env`，那个文件里镜像、端口、卷全是写死的值。
...
其余变量、默认值与含义见 README 的「配置项（环境变量）」一节。`.env` 里是明文密码，权限收紧到 `600`，只有 `deploy.sh` 会读它（生效顺序是**命令行 > 环境变量 > `.env` > 默认值**）。
...
源码装到 `./dat`，也就是你执行这条命令时所在目录下的 `dat/`。要换目录或换端口得走 `bash -s --`——管道执行时 stdin 已经被脚本占着，参数直接跟在 `| bash` 后面是传不进去的：
...
权限方面注意一件事：一键安装要往 `./dat`（或 `--root` 指的目录）写文件，而 `curl | bash` 把 stdin 占住了，`sudo` 真要密码是输不进去的。所以要么整个用 root 跑（`curl … | sudo bash`），要么 `--root` 指一个你有写权限的目录，要么给当前用户配免密 sudo。Docker 那一侧不同：当前用户不在 `docker` 组时脚本会自己在 `docker` 命令前面加 sudo。
...
脚本自己起容器（`docker build` 或 `docker load` 拿到镜像，再 `docker run` 起来），跑起来会看到这些行，含义依次是：
...
| `项目目录：…` / `运行模式：本机已有项目`（或 `运行模式：一键安装（源码本次下载）`）/ `镜像：local/nasphere:1.0.0` / `容器：nasphere` / `端口：18086 → 容器内 18086` / `数据：…/data` | 生效的参数，优先级是**命令行 > 环境变量 > `.env` > 默认值**；数据目录已转成绝对路径。上面还会有一行 `读取 …/.env`，只在项目里放了 `.env` 时出现 |
| `首次启动：账号 admin` | 部署前 `data/auth.json` 不存在时才打印（取样在容器启动之前，不会被服务端补写文件盖掉） |
| `部署方式：docker run` | 脚本一律自己用 `docker run` 起容器：`docker-compose.yml` 是给手工路线用的静态文件，镜像名、端口、卷都写死了，喂不进本次算好的参数 |
...
| `! 找不到 Docker Socket：/var/run/docker.sock` + `! NASphere Docker 管理功能将不可用` | 宿主机上找不到套接字，跳过挂载——容器组件就没数据，其余功能照常，需要的话按 §11 处理 |
...
`--dry-run` 只打印将要执行的命令（`docker build` / `docker rm` / `docker run` 那几条，`NAV_PASSWORD` 有值时打成 `***`）就结束，不构建、不备份、不起容器；一键安装模式下连源码都不下载，末尾是 `✓ dry-run 完成，没有下载、没有落盘`。
...
`docker-compose.yml` 现在只描述「用哪个镜像、开哪个端口、挂哪两个卷」，不再内嵌 Dockerfile，也不读 `.env`——**镜像得先有**。在本机 build：
...
或者直接用 §5 的离线包在 NAS 上 `docker load -i dist/nasphere-1.0.0.tar.gz`（包里的名字就是 `local/nasphere:1.0.0`，不用重新打标签）。
...
要改端口、数据目录或镜像版本，就直接改 `docker-compose.yml` 里那三行写死的值（`image`、`ports` 冒号左边、`volumes` 冒号左边），再 `docker compose up -d`。首启密码这条路线给不进去，容器会用镜像内置的默认值，登录后立刻去「设置 → 安全」改（§6、§12）。

> 这条路和 §3 是二选一：`deploy.sh` 自己用 `docker run` 起的那个容器也叫 `nasphere`，compose 再 up 会撞容器名。先 `docker rm -f nasphere` 再走这条，反之也一样。
...
不想让脚本插手的话，`docker load -i` 那份包再 `docker compose up -d` 就是 §4 那条手工路线——包里的 `RepoTags` 正是 `local/nasphere:<版本>`，与 `docker-compose.yml` 里写死的镜像名对得上。
...
1. **确认账号与密码**：设置（按 `,`）→ 安全。§2 里设过 `NAV_PASSWORD` 的，首启密码就是它、不会提示；没设而 `deploy.sh` 首装的话，密码是随机生成那一个（终端只打印过一次，留档在 `.env`，§3）；绕过 `deploy.sh` 走 §4 的手工 compose（或 §5 里 `docker load` + `docker compose up -d`），首启就一定是镜像内置的 `admin123`——这种情况下容器日志会打警告、每次打开页面也会弹提示，账号还是默认的 `admin` 时「安全」那一栏同样标出来。不管哪种，首启后就把账号和密码改掉。改过一次之后 `data/auth.json` 就是唯一凭据来源，`.env` 里的变量不再起作用（§10）。
...
docker rm -f nasphere                       # 手工 compose 起的那就用 docker compose down
cp -a data /volume2/docker/nasphere-data
./deploy.sh --data-dir /volume2/docker/nasphere-data
```

走路线 B 手工 compose 的话，改的是 `docker-compose.yml` 本身：`ports` 冒号左边那一位，和 `volumes` 第一行冒号左边的宿主目录（右边那个 `/app/data` 是容器内的挂载点，跟镜像里 `ENV DATA_DIR` 对上了才读得到数据），再 `docker compose up -d`。改完记得同步反向代理里的目标端口。
...
- **手动**：脚本每次都会把当时的在用镜像额外打个 `<IMAGE>:rollback` 标签（默认就是 `ghcr.io/peekaboo789/nasphere:rollback`），所以
  ...
  手工 compose 起的那台改成 `docker compose up -d` 收尾（`docker-compose.yml` 里写的就是这个标签）。
...
# 用脚本重装一次，凭据从环境变量取（起完之后这两个变量可以从 .env 里删掉）
NAV_USER='新账号' NAV_PASSWORD='新密码' ./deploy.sh
```

不想跑脚本就直接 `docker run`，把这两个值用 `-e` 带上（参数照 §9 那段，加上 `-e NAV_USER='新账号' -e NAV_PASSWORD='新密码'`）。手工 compose 那条路线读不到环境变量，只能先按 §6 用内置默认密码登进去、再在「设置 → 安全」里改。
...
| 现象 | 先查 | 处置 |
| --- | --- | --- |
| `这台机器上没有 docker` | 脚本要求 PATH 里有 docker | 用 Docker 图形套件里的 Compose/项目功能：先弄出 `local/nasphere:1.0.0` 这个镜像（§5 的离线包导入，或在套件里按 `Dockerfile` build 一份），再把 §4 那份 `docker-compose.yml` 贴进项目起起来；卷路径写绝对路径。或按 README「本地开发调试」直接跑 `node server/index.js` |
...
| `docker.sock: connect version mismatch` 或组件全显示「Docker 不可用」 | 套接字没挂进来、路径不是 `/var/run/docker.sock`、或版本过旧 | `deploy.sh` 路线：`.env` 里改 `DOCKER_SOCK` 后重跑。手工 compose 路线：改 `docker-compose.yml` 里那行 `- /var/run/docker.sock:/var/run/docker.sock`（左右两边保持同一个宿主机路径最省事）。不需要这能力就把那行注释掉，其余功能照常 |
...
| 非 root 起不来（`EACCES`） | 数据目录属主和容器运行用户不匹配。`deploy.sh` 起的容器是 root，`docker-compose.yml` 里也没写 `user:`，所以这条只在你自己加过运行用户时才会撞上 | `chown -R 1000:1000 data`，或者去掉自己加的那个 `user:` |
...
- 首启密码由 `deploy.sh` 随机生成（不再是人人一样的默认值），只打印一次并留档在 `.env`（`600`）。走 §4 的手工 compose（或 §5 的 `docker load` + `compose up`）拿不到这个保护——那个 compose 文件不读 `.env`，首启必定是镜像内置的默认密码，而容器还挂着 `docker.sock`，裸密码等于把宿主机一起送出去。所以那条路线起来后第一件事就是登进去改密码；想连这一步都省掉，就还是用 `deploy.sh`。
...
```bash
# 状态与日志（deploy.sh 起的容器是 docker run 出来的，用 docker 这套看；手工 compose 起的用 docker compose ps / logs）
docker ps -a --filter name=nasphere ; docker logs -f --tail 50 nasphere
docker inspect -f '{{.State.Health.Status}}' nasphere

# 只重启（换配置不重建镜像）
docker restart nasphere

# 彻底重来（数据保留！只是删容器）
docker rm -f nasphere ; ./deploy.sh
